...
detect_platform() {
  ...
  case "$os" in
    Darwin) os="darwin" ;;
    *) error "unsupported OS: $os (gitnagg is macOS-only)" ;;
  esac
...
main() {
  ...
  if [ -n "$current" ] && [ "$current" = "$clean_version" ] && [ -z "$FORCE" ]; then
    ...
    info "  curl -fsSL https://raw.githubusercontent.com/Ryu0118/gitnagg/main/install.sh | FORCE=1 bash"
    exit 0
  fi
